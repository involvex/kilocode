 ...
 */
function getContextWindowFromModel(apiConfig, routerModels) {
 ...
 */
function getContextTokensFromMessages(messages) {
	...
	for (let i = messages.length - 1; i >= 0; i--) {
		...
		if (message.type === "say" && message.say === "api_req_started" && message.text) {
			...
			const contextCondense = message.metadata
 ...
 */
export function calculateTokenDistribution(contextWindow, contextTokens, maxTokens) {
 ...
 */
export function calculateContextUsage(messages, apiConfig, routerModels = null) {
	try {
