...
export class ExtensionHost extends EventEmitter {
	...
	restoreConsole() {
		...
		if (global.__interceptedConsole) {
			delete global.__interceptedConsole
		}
	...
	async loadExtension() {
		...
		try {
			...
			ModuleClass._resolveFilename = function (request, parent, isMain, options) {
				if (request === "vscode") {
					return "vscode-mock"
				}
			...
			ModuleClass.prototype._compile = function (content, filename) {
	...
	setupExtensionMessageListener() {
		...
		if (this.vscodeAPI && this.vscodeAPI.context) {
			...
			this.on("extensionWebviewMessage", (message) => {
				this.safeExecute(() => {
	 ...
	 */
	async handleLocalStateUpdates(message) {
		try {
			switch (message.type) {
	 ...
	 */
	async syncConfigurationMessages(configState) {
	 ...
	 */
	async injectConfiguration(configState) {
		if (!this.currentState) {
