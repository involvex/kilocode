...
describe("ExtensionHost Race Condition Fix", () => {
	...
	beforeEach(() => {
		...
		const initializeState = extensionHost.initializeState.bind(extensionHost)
		initializeState()
		extensionHost.isActivated = true
	})
	afterEach(() => {
		vi.clearAllMocks()
	})
	describe("Message Queuing", () => {
		it("should queue messages when webview is not initialized", async () => {
			...
			const pendingMessages = extensionHost.pendingMessages
		...
		it("should not queue messages after webview is ready", async () => {
			...
			extensionHost.webviewProviders.set("kilo-code.SidebarProvider", mockProvider)
	...
	describe("Message Flushing", () => {
		it("should flush all pending messages when webview becomes ready", async () => {
			...
			expect(extensionHost.pendingMessages).toHaveLength(2)
			...
			expect(extensionHost.pendingMessages).toHaveLength(0)
		...
		it("should maintain message order when flushing", async () => {
			...
			const callOrder = []
			const mockProvider = {
				handleCLIMessage: vi.fn(async (msg) => {
