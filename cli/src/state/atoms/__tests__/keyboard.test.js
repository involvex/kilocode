...
import { historyDataAtom, historyModeAtom } from "../history.js"
describe("keypress atoms", () => {
	...
	describe("submission callback", () => {
		...
		it("should handle non-function callback gracefully", () => {
			...
			store.set(submissionCallbackAtom, { callback: "not a function" })
			// Type 'hello'
			const chars = ["h", "e", "l", "l", "o"]
			for (const char of chars) {
				const key = {
		...
		it("should convert Buffer to string when submitting", () => {
			...
			store.set(submitInputAtom, buffer)
