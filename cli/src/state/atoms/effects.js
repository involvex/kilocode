 ...
 */
const messageBufferAtom = atom([])
/**
 * Flag to track if we're currently processing buffered messages
 */
const isProcessingBufferAtom = atom(false)
/**
...
export const initializeServiceEffectAtom = atom(null, async (get, set, store) => {
	...
	const atomStore = store || get.store
	if (!atomStore) {
 ...
 */
export const messageHandlerEffectAtom = atom(null, (get, set, message) => {
	try {
		...
		switch (message.type) {
			...
			case "routerModels": {
				const routerModels = message.routerModels
				if (routerModels) {
			...
			case "profileDataResponse": {
				...
				if (payload?.success) {
					set(updateProfileDataAtom, payload.data)
				} else {
			...
			case "balanceDataResponse": {
				...
				const payload = message.payload
				if (payload?.success) {
					set(updateBalanceDataAtom, payload.data)
				} else {
			...
			case "taskHistoryResponse": {
				...
				if (payload) {
					...
					const payloadWithRequestId = message.payload
			...
			case "commandExecutionStatus": {
				...
				try {
					const statusData = JSON.parse(message.text || "{}")
					...
					if (statusData.status === "started") {
						...
						const command = "command" in statusData ? statusData.command : undefined
						const updateData = {
							output: "",
						...
						const command = "command" in statusData ? statusData.command : existing.command
						const updateData = {
						...
						const command = existing.command || ("command" in statusData ? statusData.command : "")
						const finalUpdate = {
							...existing,
							command: command,
							completed: true,
...
export const disposeServiceEffectAtom = atom(null, async (get, set) => {
	...
	try {
		...
		set(pendingOutputUpdatesAtom, new Map())
		// Clear command output ask tracking
		set(commandOutputAskShownAtom, new Map())
		// Dispose the service
 ...
 */
export const messageBufferSizeAtom = atom((get) => {
	const buffer = get(messageBufferAtom)
 ...
 */
export const hasBufferedMessagesAtom = atom((get) => {
