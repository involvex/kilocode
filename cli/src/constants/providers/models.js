 ...
 */
export const PROVIDER_TO_ROUTER_NAME = {
	kilocode: "kilocode",
 ...
 */
export const PROVIDER_MODEL_FIELD = {
 ...
 */
export function getModelIdKey(provider) {
	switch (provider) {
		case "openrouter":
...
export function getCurrentModelId(params) {
	...
	if (provider === "vscode-lm" && providerConfig.vsCodeLmModelSelector) {
		const selector = providerConfig.vsCodeLmModelSelector
		return `${selector?.vendor}/${selector?.family}`
	}
	// Get model ID from config
	const modelId = providerConfig[modelIdKey]
 ...
 */
export function sortModelsByPreference(models) {
 ...
 */
export function formatModelInfo(modelId, model) {
	const parts = []
	// Context window
 ...
 */
export function fuzzyFilterModels(models, filter) {
	if (!filter) {
