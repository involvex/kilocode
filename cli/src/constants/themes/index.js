 ...
 */
export function getThemeById(themeId, config) {
...
export function getAvailableThemes(config) {
	...
	if (config && config.customThemes) {
		const customThemeIds = Object.keys(config.customThemes)
	...
	return allThemes.sort((a, b) => {
		...
		const typeAOrder = typeOrder[themeA.type] ?? 3
		const typeBOrder = typeOrder[themeB.type] ?? 3
 ...
 */
export function isValidThemeId(themeId, config) {
	const builtInThemes = Object.keys(themeRegistry)
