 ...
 */
function validateRequiredField(provider, fieldName, errors) {
 ...
 */
function handleSpecialValidations(provider, errors) {
	switch (provider.provider) {
		case "vertex": {
			// At least one of vertexJsonCredentials or vertexKeyFile must be provided
			const jsonCreds = provider.vertexJsonCredentials
			const keyFile = provider.vertexKeyFile
		...
		case "vscode-lm": {
			const selector = provider.vsCodeLmModelSelector
			if (!selector) {
 ...
 */
export function validateProviderConfig(provider, isSelected = false) {
 ...
 */
export function validateSelectedProvider(config) {
