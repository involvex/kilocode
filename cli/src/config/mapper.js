...
export function mapConfigToExtensionState(config, currentState) {
	try {
		...
		const listApiConfigMeta = config.providers.map((p) => ({
			id: p.id,
			name: p.id,
...
function mapProviderToApiConfig(provider) {
	const config = {
...
export function getModelIdForProvider(provider) {
...
export function mapExtensionStateToConfig(state, currentConfig) {
