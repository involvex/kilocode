 ...
 */
const calculateAdaptiveSpeed = (timeSinceLastChunk, remainingChars) => {
 ...
 */
export const MarkdownText = ({ children, ...options }) => {
	 ...
	 */
	const startAnimation = useCallback((intervalMs) => {
		if (animationTimerRef.current) {
			clearInterval(animationTimerRef.current)
		}
	...
	try {
		...
		const rendered = parse(textToDisplay)
