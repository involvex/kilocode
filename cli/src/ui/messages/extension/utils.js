 ...
 */
export function parseMessageJson(text) {
	if (!text) return null
	try {
 ...
 */
export function parseToolData(message) {
	return parseMessageJson(message.text)
}
/**
 ...
 */
export function parseFollowUpData(message) {
	// Try parsing from text first
	const fromText = parseMessageJson(message.text)
	...
	if (message.metadata) {
		...
		if (typeof message.metadata === "string") {
			return parseMessageJson(message.metadata)
		}
	}
	return null
}
 ...
 */
export function parseApiReqInfo(message) {
	return parseMessageJson(message.text)
}
/**
 * Parse image data from message
 */
export function parseImageData(message) {
	return parseMessageJson(message.text)
}
/**
 * Get icon for message type
 */
export function getMessageIcon(type, subtype) {
	if (type === "ask") {
		switch (subtype) {
			case "tool":
				return "⚙"
 ...
 */
export function getMessageColor(type, subtype) {
	if (type === "ask") {
		return "yellow"
	}
	switch (subtype) {
 ...
 */
export function hasJsonContent(message) {
...
export function formatContentWithMetadata(text, maxLines = 20, previewLines = 5) {
	if (!text) {
		return {
			isJson: false,
			content: "",
