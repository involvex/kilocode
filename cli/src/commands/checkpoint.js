 ...
 */
function getCheckpointMessages(chatMessages) {
	return chatMessages
		.filter(
			(msg) =>
				msg.type === "say" &&
 ...
 */
function findCheckpointByHash(checkpoints, hash) {
 ...
 */
async function handleRestore(context, hash) {
 ...
 */
async function provideCheckpointHashes(context) {
 ...
 */
async function provideSubcommands(context) {
	const subcommands = [
