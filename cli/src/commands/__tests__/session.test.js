...
describe("sessionCommand", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(SessionManager.init).mockReturnValue(mockSessionManager)
	})
	afterEach(() => {
		vi.clearAllMocks()
	})
	...
	describe("handler - no arguments", () => {
		it("should show usage message when called without arguments", async () => {
			...
			const message = mockContext.addMessage.mock.calls[0][0]
	...
	describe("handler - select subcommand", () => {
		it("should restore session successfully", async () => {
			...
			const replacedMessages = mockContext.replaceMessages.mock.calls[0][0]
		...
		it("should handle restore error gracefully", async () => {
			...
			const errorMessage = mockContext.addMessage.mock.calls.find((call) => call[0].type === "error")
	...
	describe("sessionIdAutocompleteProvider", () => {
		...
		const createProviderContext = (partialInput, subcommand) => ({
			commandName: "session",
			argumentIndex: 1,
			...
			currentOptions: {},
			partialInput,
			getArgument: (name) => (name === "subcommand" ? subcommand : undefined),
