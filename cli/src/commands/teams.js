 ...
 */
async function listTeams(context) {
 ...
 * Select a team
 */
async function selectTeam(context, teamId) {
	...
	try {
		...
		if (profileData?.organizations) {
			...
			let targetOrg = profileData.organizations.find((org) => org.id === teamId)
			// If not found by ID, try normalized name match
			if (!targetOrg) {
				const normalizedInput = normalizeTeamName(teamId)
				targetOrg = profileData.organizations.find((org) => normalizeTeamName(org.name) === normalizedInput)
			}
			if (!targetOrg) {
				addMessage({
					id: Date.now().toString(),
 ...
 */
async function teamAutocompleteProvider(context) {
