 ...
 */
async function showTaskHistory(context, dataOverride) {
 ...
 * Search tasks
 */
async function searchTasks(context, query) {
 ...
 * Select a task by ID
 */
async function selectTask(context, taskId) {
 ...
 * Change page
 */
async function changePage(context, pageNum) {
 ...
 * Go to previous page
 */
async function previousPage(context) {
 ...
 * Change sort order
 */
async function changeSortOrder(context, sortOption) {
	const { updateTaskHistoryFilters, addMessage } = context
	...
	try {
		// Wait for the new data to arrive
		const newData = await updateTaskHistoryFilters({ sort: mappedSort })
		// Now display the fresh data
		await showTaskHistory(context, newData)
	} catch (error) {
		addMessage({
 ...
 * Change filter
 */
async function changeFilter(context, filterOption) {
	const { updateTaskHistoryFilters, addMessage } = context
	let filterUpdate
	let loadingMessage
	switch (filterOption) {
 ...
 */
async function taskIdAutocompleteProvider(context) {
	...
	return taskHistoryData.historyItems.map((task) => ({
		value: task.id,
