 ...
 */
async function themeAutocompleteProvider(context) {
	...
	const sortedThemes = availableThemeIds
		...
		.sort((a, b) => {
			...
			const typeAOrder = typeOrder[a.type] ?? 3
			const typeBOrder = typeOrder[b.type] ?? 3
 ...
 */
function getThemeDisplayInfo(config) {
	...
	handler: async (context) => {
		...
		try {
			...
			if (args.length === 0 || !args[0]) {
				...
				const themesByType = allThemes.reduce((acc, theme) => {
					if (!acc[theme.type]) {
						acc[theme.type] = []
					}
				...
				const helpText = ["**Available Themes:**", ""]
