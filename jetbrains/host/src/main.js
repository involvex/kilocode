...
import { SocketCloseEventType } from "../deps/vscode/vs/base/parts/ipc/common/ipc.net.js"
...
const server = net.createServer((socket) => {
	...
	const closeDisposable = nodeSocket.onClose((event) => {
	...
	protocol.onMessage((message) => {
		if (isMessageOfType(message, MessageType.Ready)) {
			...
			if (rpcProtocol) {
				extensionManager.activateExtension(rooCodeIdentifier.value, rpcProtocol).catch((error) => {
...
function startExtensionHostProcess() {
	...
	extHostProcess = fork(path.join(__dirname, "extension.js"), [], {
		env: {
			...
			VSCODE_EXTHOST_SOCKET_PORT: server.address()?.port?.toString() || "0",
