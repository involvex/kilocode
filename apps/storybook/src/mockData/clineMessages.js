 ...
 */
export const createComponentCreationMessages = () => {
 ...
 */
export const createDebuggingMessages = () => {
	let currentTime = BASE_TIMESTAMP
	const messages = [
		{
			ts: currentTime,
 ...
 */
export const createFullStackMessages = () => {
	let currentTime = BASE_TIMESTAMP
	const messages = [
		{
			ts: currentTime,
	...
	for (let i = 1; i < messageCount && i < messageTypes.length + 1; i++) {
		...
		messages.push({
			ts: currentTime,
			type: msgType.type,
			...(msgType.ask && { ask: msgType.ask }),
			...(msgType.say && { say: msgType.say }),
			text:
...
export const createTaskHeaderMessages = () => {
	...
	const addMessage = (message, timingRange = [800, 1200]) => {
 ...
 */
export const createQuickTaskMessages = () => [
	{
		ts: BASE_TIMESTAMP,
		type: "ask",
		ask: "command",
 ...
 */
export const createMessageTypeVarietyMessages = () => {
	let currentTime = BASE_TIMESTAMP
	const messages = []
	// Helper to add message with timing
	const addMessage = (message, timingRange = [1000, 3000]) => {
		messages.push({ ...message, ts: currentTime })
		currentTime += randomInterval(timingRange[0], timingRange[1])
	}
