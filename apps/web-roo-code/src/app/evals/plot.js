...
export const Plot = ({ tableData }) => {
	const chartData = useMemo(() => tableData.filter(({ cost }) => cost < 50), [tableData])
	const chartConfig = useMemo(() => chartData.reduce((acc, run) => ({ ...acc, [run.label]: run }), {}), [chartData])
	...
	const labelPositions = useMemo(() => {
		...
		const wouldLabelsOverlap = (p1, p2) => {
		...
		const wouldOverlapPoint = (point, position) => {
		...
		sortedData.forEach((point) => {
			...
			const positionPreferences = ["top", "bottom", "right", "left"]
			...
			<ChartContainer config={chartConfig} className="h-[500px] w-full">
				<ScatterChart margin={{ top: 20, right: 0, bottom: 0, left: 20 }}>
					<XAxis
						...
						name="Cost"
						domain={[
							(dataMin) => Math.max(0, Math.round((dataMin - 5) / 5) * 5),
							(dataMax) => Math.round((dataMax + 5) / 5) * 5,
						]}
					...
					<YAxis
						...
						domain={[
							(dataMin) => Math.max(0, Math.round((dataMin - 5) / 5) * 5),
							(dataMax) => Math.min(100, Math.round((dataMax + 5) / 5) * 5),
						]}
...
const generateSpectrumColor = (index, total) => {
