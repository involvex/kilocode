...
suite.skip("Roo Code read_file Tool", function () {
	...
	test("Should read a simple text file", async function () {
		...
		try {
			...
			await (0, utils_1.waitFor)(() => taskStarted, { timeout: 60_000 })
			// Check for early errors
			if (errorOccurred) {
				console.error("Early error detected:", errorOccurred)
			}
			// Wait for task completion
			await (0, utils_1.waitFor)(() => taskCompleted, { timeout: 60_000 })
			...
			const actualContent = toolResult.replace(/^\d+\s*\|\s*/, "")
			assert.strictEqual(
				actualContent.trim(),
				"Hello, World!",
	...
	test("Should read file with line range", async function () {
		...
		try {
			...
			if (toolResult && toolResult.includes(" | ")) {
				// The result includes line numbers
				assert.ok(toolResult.includes("2 | Line 2"), "Tool result should include line 2 with line number")
				assert.ok(toolResult.includes("3 | Line 3"), "Tool result should include line 3 with line number")
				assert.ok(toolResult.includes("4 | Line 4"), "Tool result should include line 4 with line number")
			}
