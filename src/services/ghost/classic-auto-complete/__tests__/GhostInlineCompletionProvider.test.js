...
describe("GhostInlineCompletionProvider", () => {
	...
	async function provideWithDebounce(doc, pos, ctx, token) {
	...
	beforeEach(() => {
		...
		vi.spyOn(GhostContextProvider.prototype, "getIde").mockReturnValue(mockIde)
	...
	describe("provideInlineCompletionItems", () => {
		...
		it("should update suggestions when called multiple times", async () => {
			...
			let result = await provideWithDebounce(mockDocument, mockPosition, mockContext, mockToken)
			...
			result = await provideWithDebounce(mockDocument, mockPosition, mockContext, mockToken)
		...
		it("should maintain a rolling window of suggestions and match from most recent", async () => {
			...
			result = await provideWithDebounce(mockDocument2, mockPosition2, mockContext, mockToken)
			expect(result[0].insertText).toBe("second suggestion")
		})
		...
		it("should maintain only the last 20 suggestions (FIFO)", async () => {
			...
			let result = await provideWithDebounce(mockDocument0, mockPosition0, mockContext, mockToken)
			expect(result).toHaveLength(0)
			...
			result = await provideWithDebounce(mockDocument10, mockPosition10, mockContext, mockToken)
			...
			result = await provideWithDebounce(mockDocument24, mockPosition24, mockContext, mockToken)
		...
		it("should allow same text with different prefix/suffix", async () => {
			...
			const mockDocument2 = new MockTextDocument(vscode.Uri.file("/test2.ts"), "const a = 1\nconst b = 2")
			const mockPosition2 = new vscode.Position(0, 11)
			const result = await provideWithDebounce(mockDocument2, mockPosition2, mockContext, mockToken)
		...
		describe("partial typing support", () => {
			it("should return remaining suggestion when user has partially typed the suggestion", async () => {
				...
				const result = await provideWithDebounce(partialDocument, partialPosition, mockContext, mockToken)
				expect(result).toHaveLength(1)
			...
			it("should return empty when partially typed content does not match suggestion", async () => {
				...
				const result = await provideWithDebounce(mismatchDocument, mismatchPosition, mockContext, mockToken)
				expect(result).toHaveLength(0)
			})
			it("should return empty string when user has typed entire suggestion", async () => {
				...
				const result = await provideWithDebounce(completeDocument, completePosition, mockContext, mockToken)
			...
			it("should prefer exact match over partial match", async () => {
				...
				const result = await provideWithDebounce(document, position, mockContext, mockToken)
				expect(result).toHaveLength(1)
	...
	describe("failed lookups cache", () => {
		it("should cache failed LLM lookups and not call LLM again for same prefix/suffix", async () => {
			...
			const result1 = await provideWithDebounce(mockDocument, mockPosition, mockContext, mockToken)
			...
			const result2 = await provideWithDebounce(mockDocument, mockPosition, mockContext, mockToken)
	...
	describe("untitled document handling", () => {
		it("should provide completions for untitled documents", async () => {
			...
			const result = await provideWithDebounce(untitledDocument, mockPosition, mockContext, mockToken)
...
//# sourceMappingURL=GhostInlineCompletionProvider.test.js.map
