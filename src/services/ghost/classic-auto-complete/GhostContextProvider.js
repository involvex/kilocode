...
export class GhostContextProvider {
	...
	model
	ignoreController
	constructor(context, model, ignoreController) {
	...
	async filterSnippetsByAccess(snippets) {
	...
	async getProcessedSnippets(autocompleteInput, filepath) {
		...
		const helper = await HelperVars.create(helperInput, DEFAULT_AUTOCOMPLETE_OPTS, modelName, this.ide)
		...
		const snippetsWithUris = accessibleSnippets.map((snippet) => ({
			...snippet,
...
//# sourceMappingURL=GhostContextProvider.js.map
