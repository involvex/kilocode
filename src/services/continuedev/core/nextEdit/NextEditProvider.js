...
export class NextEditProvider {
	...
	modelProvider = null
	constructor(configHandler, ide, _injectedGetLlm, _onError, getDefinitionsFromLsp, _endpointType) {
	...
	async _handleCompletion(helper, prompts, token, startTime, editableRegionStartLine, editableRegionEndLine, opts) {
		...
		const msg = await llm.chat([prompts[1]], token, {
			stream: false,
		})
...
export function __setMockNextEditProviderInstance(mockInstance) {
	NextEditProvider._instance = mockInstance
}
