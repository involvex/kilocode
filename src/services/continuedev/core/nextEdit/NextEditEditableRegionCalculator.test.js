...
describe("NextEditEditableRegionCalculator", () => {
	describe("staticJump - Tree-sitter AST Navigation", () => {
		let mockIde
		let mockGetMostRecentAst
		beforeEach(() => {
			// Reset mocks
			mockGetMostRecentAst = vi.fn()
			DocumentHistoryTracker.getInstance.mockReturnValue({
  ...
  return name;
			...
			const cursorPosition = { line: 1, character: 15 }
			...
			mockIde.getReferences.mockResolvedValue(mockReferences)
  ...
  return value;  // refers to outer value
			...
			const cursorPosition = { line: 4, character: 10 }
    ...
    def main(self):
        ...
        return result`
			...
			const cursorPosition = { line: 5, character: 12 }
			const mockReferences = [
				{
					filepath,
					range: {
  ...
  if (true) {
    ...
    while (temp > 0) {
      ...
      console.log(value);
			...
			const cursorPosition = { line: 4, character: 12 }
			const mockReferences = [
				{
					filepath,
					range: {
...
console.log(myVar);`
			...
			const cursorPosition = { line: 1, character: 13 }
			const mockReferences = [
				{
					filepath,
					range: {
    ...
    def multiply(self, x, y):
        ...
        return result * 2`
			...
			const cursorPosition = { line: 5, character: 21 }
			const mockReferences = [
				{
					filepath,
					range: {
		...
		it("should return null when no identifier found at cursor", async () => {
			...
			const cursorPosition = { line: 1, character: 5 }
			const result = await getNextEditableRegion(EditableRegionStrategy.Static, {
				cursorPosition,
				filepath,
				ide: mockIde,
		...
		it("should return null when no references found", async () => {
			...
			const tree = await getAst(filepath, code)
			mockGetMostRecentAst.mockResolvedValue(tree)
			const cursorPosition = { line: 0, character: 6 }
			mockIde.getReferences.mockResolvedValue([])
			const result = await getNextEditableRegion(EditableRegionStrategy.Static, {
				cursorPosition,
				filepath,
				ide: mockIde,
...
count++;  // Refers to outer count`
			...
			const cursorPosition = { line: 3, character: 6 }
		...
		it("should handle IDE.getReferences throwing an error", async () => {
			...
			mockGetMostRecentAst.mockResolvedValue(tree)
			const cursorPosition = { line: 0, character: 6 }
			mockIde.getReferences.mockRejectedValue(new Error("IDE error"))
			const result = await getNextEditableRegion(EditableRegionStrategy.Static, {
				cursorPosition,
				filepath,
				ide: mockIde,
...
//# sourceMappingURL=NextEditEditableRegionCalculator.test.js.map
