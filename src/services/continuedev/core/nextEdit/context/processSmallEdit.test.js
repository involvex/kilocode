...
describe("processSmallEdit", () => {
	let mockEditAggregator
	let mockNextEditProvider
	let mockIde
	let mockConfigHandler
	let mockGetDefsFromLspFunction
	beforeEach(() => {
		vi.clearAllMocks()
		...
		EditAggregator.getInstance.mockReturnValue(mockEditAggregator)
		// Setup mock NextEditProvider
		mockNextEditProvider = {
			addDiffToContext: vi.fn(),
		}
		NextEditProvider.getInstance.mockReturnValue(mockNextEditProvider)
		processNextEditData.mockResolvedValue(undefined)
		// Setup mock IDE
		mockIde = {
	...
	const createMockBeforeAfterDiff = () => ({
	...
	describe("default context handling", () => {
		...
		it("should use provided configHandler when no context data", async () => {
			...
			const call = processNextEditData.mock.calls[0][0]
	...
	describe("cursor position handling", () => {
		it("should pass different cursor positions correctly", async () => {
			const diff = createMockBeforeAfterDiff()
			const beforePos = { line: 5, character: 10 }
			const afterPos = { line: 6, character: 15 }
		...
		it("should handle same cursor position before and after", async () => {
			const diff = createMockBeforeAfterDiff()
			const samePos = { line: 10, character: 5 }
	...
	describe("async behavior", () => {
		it("should not wait for processNextEditData to complete", async () => {
			...
			processNextEditData.mockReturnValue(delayedPromise)
			const diff = createMockBeforeAfterDiff()
...
//# sourceMappingURL=processSmallEdit.test.js.map
