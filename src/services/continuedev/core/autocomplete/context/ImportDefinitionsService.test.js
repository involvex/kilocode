...
import sys`
			...
			mockIde.readFile.mockResolvedValue(pythonCode)
			...
			mockIde.gotoDefinition.mockResolvedValue([
				{
					filepath: mockDefinition.filepath,
					range: mockDefinition.range,
				},
			])
			mockIde.readRangeInFile.mockResolvedValue(mockDefinition.contents)
			const result = await service._getFileInfo(filepath)
...
'''`
			...
			mockIde.readRangeInFile.mockResolvedValue("class MyClass: ...")
			const result = await service._getFileInfo(filepath)
...
import json`
			...
			mockIde.readFile.mockResolvedValue(pythonCode)
			mockIde.gotoDefinition.mockResolvedValue([])
			mockIde.readRangeInFile.mockResolvedValue("")
			const result = await service._getFileInfo(filepath)
...
import defaultExport from './module';`
			...
			mockIde.readFile.mockResolvedValue(tsCode)
			mockIde.gotoDefinition.mockResolvedValue([
				{
			...
			mockIde.readRangeInFile.mockResolvedValue("export const Component = ...")
			const result = await service._getFileInfo(filepath)
...
\`;`
			...
			mockIde.readRangeInFile.mockResolvedValue("export const realImport = ...")
			const result = await service._getFileInfo(filepath)
...
import defaultExport from './module';`
    ...
    print("Hello, World!")`
		...
		it("should handle syntax errors gracefully", async () => {
			...
			mockIde.readFile.mockResolvedValue(badCode)
			const result = await service._getFileInfo(filepath)
...
def func():
	...
	describe("Verify Tree-sitter Dependency", () => {
		it("should fail if tree-sitter cannot parse TypeScript", async () => {
			...
			mockIde.readFile.mockResolvedValue(code)
			const result = await service._getFileInfo(filepath)
...
//# sourceMappingURL=ImportDefinitionsService.test.js.map
