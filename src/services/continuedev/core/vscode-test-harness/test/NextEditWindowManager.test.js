...
vi.mock("vscode", () => ({
	...
	Uri: {
		parse: vi.fn((str) => ({ toString: () => str, scheme: "file" })),
	},
...
} from "../src/activation/NextEditWindowManager"
...
describe("NextEditWindowManager", () => {
	...
	beforeEach(() => {
		...
		mockDocument = {
			...
			lineAt: vi.fn((line) => ({
				...
				range: new mockVscode.Range(new mockVscode.Position(line, 0), new mockVscode.Position(line, 20)),
			})),
		}
		// Create mock editor
		mockEditor = {
		...
		mockVscode.commands.registerCommand
			...
			.mockImplementation((_command, _callback) => ({
				dispose: vi.fn(),
			}))
		mockVscode.window.createTextEditorDecorationType.mockReturnValue({
			dispose: vi.fn(),
		})
	...
	describe("Key Reservation State Management", () => {
		...
		it("should reserve keys when showing window", async () => {
			await manager.showNextEditWindow(mockEditor, new mockVscode.Position(0, 0), 0, 0, "old", "new", [])
			expect(mockVscode.commands.executeCommand).toHaveBeenCalledWith("setContext", "nextEditWindowActive", true)
		...
		it("should free keys when hiding windows", async () => {
			...
			expect(mockVscode.commands.executeCommand).toHaveBeenCalledWith("setContext", "nextEditWindowActive", false)
			expect(manager["keyReservationState"]).toBe("free")
		})
		it("should handle sequential reserve/free operations", async () => {
			const executeCommand = mockVscode.commands.executeCommand
	...
	describe("Race Condition Prevention", () => {
		...
		it("should handle rapid fire operations correctly (loop)", async () => {
			...
			for (let i = 0; i < 20; i++) {
				operations.push(
					i % 2 === 0 ? NextEditWindowManager.reserveTabAndEsc() : NextEditWindowManager.freeTabAndEsc(),
				)
			}
			await Promise.all(operations)
		...
		it("should maintain correct state with stale operations", async () => {
			...
			executeCommand.mockImplementationOnce(() => firstPromise).mockImplementationOnce(() => secondPromise)
			// Start operations
			const op1 = NextEditWindowManager.reserveTabAndEsc()
			const op2 = NextEditWindowManager.freeTabAndEsc()
	...
	describe("Error Recovery", () => {
		...
		it("should recover from failed key reservation", async () => {
			...
			await expect(NextEditWindowManager.reserveTabAndEsc()).rejects.toThrow("VS Code busy")
		...
		it("should recover from failed key freeing", async () => {
			...
			// Make freeing fail
			executeCommand.mockRejectedValueOnce(new Error("VS Code busy"))
			await expect(NextEditWindowManager.freeTabAndEsc()).rejects.toThrow("VS Code busy")
		...
		it("should handle decoration creation failure", async () => {
			const consoleErrorSpy = vi.spyOn(console, "error").mockImplementation(() => {})
			const createDecoration = mockVscode.window.createTextEditorDecorationType
	...
	describe("Command Registration", () => {
		...
		it("should handle command registration failure gracefully", async () => {
			const registerCommand = mockVscode.commands.registerCommand
			registerCommand.mockImplementationOnce(() => {
				throw new Error("Command already registered")
			})
			// Should not throw
			await expect(manager.setupNextEditWindowManager(mockContext)).resolves.not.toThrow()
		})
	})
	describe("Window Management", () => {
		...
		it("should clear tooltip text when hiding windows", async () => {
			// Show first
			await manager.showNextEditWindow(mockEditor, new mockVscode.Position(0, 0), 0, 2, "old", "new", [])
			// Then hide
			await manager.hideAllNextEditWindows()
		...
		it("should handle line deletion case", async () => {
			mockEditor.document.lineAt = vi.fn((_lineNumber) => ({
				text: "line to delete",
			}))
			await manager.showNextEditWindow(
				mockEditor,
		...
		it("should skip rendering for excluded URIs", async () => {
			...
			expect(mockVscode.window.createTextEditorDecorationType).not.toHaveBeenCalled()
			expect(manager["currentDecoration"]).toBeNull()
		})
	})
	describe("Accept/Reject Flow", () => {
		...
		it("should apply text when accepting", async () => {
			// Show window first
			await manager.showNextEditWindow(mockEditor, new mockVscode.Position(0, 0), 0, 0, "old", "new text", [])
			// Get the accept command callback
			const registerCalls = mockVscode.commands.registerCommand.mock.calls
			const acceptCall = registerCalls.find((call) => call[0] === ACCEPT_NEXT_EDIT_SUGGESTION_COMMAND)
			// @ts-expect-error -- extracting callback from mock call array
			const acceptCallback = acceptCall[1]
			// Call accept
			// @ts-expect-error -- calling extracted callback without args
		...
		it("should hide window when rejecting", async () => {
			...
			const registerCalls = mockVscode.commands.registerCommand.mock.calls
			const hideCall = registerCalls.find((call) => call[0] === HIDE_NEXT_EDIT_SUGGESTION_COMMAND)
			// @ts-expect-error -- extracting callback from mock call array
	...
	describe("Disposal", () => {
		...
		it("should dispose decorations on disposal", async () => {
			...
			manager["currentDecoration"] = mockDecoration
	...
	describe("Configuration Changes", () => {
		it("should handle theme change", async () => {
			...
			const onDidChange = mockVscode.workspace.onDidChangeConfiguration
			const changeCallback = onDidChange.mock.calls[0][0]
			// Simulate theme change
			// @ts-expect-error -- mocking partial ConfigurationChangeEvent
			await changeCallback({
				affectsConfiguration: (key) => key === "workbench.colorTheme",
			})
		...
		it("should update on font size change", async () => {
			...
			const onDidChange = mockVscode.workspace.onDidChangeConfiguration
			const changeCallback = onDidChange.mock.calls[0][0]
			mockVscode.workspace.getConfiguration.mockReturnValueOnce({
				get: (key) => (key === "fontSize" ? 16 : "monospace"),
			})
			// @ts-expect-error -- mocking partial ConfigurationChangeEvent
			await changeCallback({
...
//# sourceMappingURL=NextEditWindowManager.test.js.map
