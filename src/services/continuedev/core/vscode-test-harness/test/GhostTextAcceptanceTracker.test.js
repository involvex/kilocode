...
import { GhostTextAcceptanceTracker } from "../src/autocomplete/GhostTextAcceptanceTracker"
// Mock the vscode module
vi.mock("vscode", () => ({
	Position: vi.fn((line, character) => ({
		line,
		character,
		isEqual: vi.fn((other) => other.line === line && other.character === character),
	})),
...
describe("GhostTextAcceptanceTracker", () => {
	...
	describe("setExpectedGhostTextAcceptance", () => {
		it("should set expected acceptance for single line text", () => {
			...
			expect(tracker.checkGhostTextWasAccepted(newDocument, endPosition)).toBe(true)
		})
	...
	describe("checkGhostTextWasAccepted", () => {
		...
		it("should return false when no expected acceptance is set", () => {
			...
			const newDocument = { ...mockDocument, version: 2 }
			const position = new vscode.Position(5, 29)
			expect(newTracker.checkGhostTextWasAccepted(newDocument, position)).toBe(false)
		})
		it("should return false when document URI doesn't match", () => {
			...
			const position = new vscode.Position(5, 29)
			expect(tracker.checkGhostTextWasAccepted(differentDocument, position)).toBe(false)
		})
		it("should return false when document version is not newer", () => {
			...
			const position = new vscode.Position(5, 29)
			expect(tracker.checkGhostTextWasAccepted(sameVersionDocument, position)).toBe(false)
			const olderVersionDocument = {
				...mockDocument,
				version: 0, // older version
			}
			expect(tracker.checkGhostTextWasAccepted(olderVersionDocument, position)).toBe(false)
		})
		it("should return false when cursor position doesn't match expected end position", () => {
			...
			expect(tracker.checkGhostTextWasAccepted(newDocument, wrongPosition)).toBe(false)
		})
		it("should return false when text content doesn't match", () => {
			...
			expect(tracker.checkGhostTextWasAccepted(newDocument, correctPosition)).toBe(false)
		})
		it("should return true when all conditions match and clear expectation", () => {
			...
			expect(tracker.checkGhostTextWasAccepted(newDocument, correctPosition)).toBe(true)
		...
		it("should work with multi-line ghost text acceptance", () => {
			...
			tracker.setExpectedGhostTextAcceptance(mockDocument, multiLineText, startPosition)
			const newDocument = {
				...mockDocument,
				version: 2,
		...
		it("should handle empty text", () => {
			...
			const emptyText = ""
			tracker.setExpectedGhostTextAcceptance(mockDocument, emptyText, startPosition)
			const newDocument = {
				...mockDocument,
				version: 2,
			...
			expect(tracker.checkGhostTextWasAccepted(newDocument, samePosition)).toBe(true)
		})
	})
})
//# sourceMappingURL=GhostTextAcceptanceTracker.test.js.map
