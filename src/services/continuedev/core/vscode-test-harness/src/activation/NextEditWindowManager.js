...
const SVG_CONFIG = {
	...
	get fontSize() {
		return Math.ceil(vscode.workspace.getConfiguration("editor").get("fontSize") ?? 14)
	},
	get fontFamily() {
...
export class NextEditWindowManager {
	static instance
	excludedURIPrefixes = ["output:", "vscode://inline-chat"]
	theme
	fontSize
	fontFamily
	codeRenderer
	...
	constructor() {
		...
		this.fontSize = editorConfig.get("fontSize") ?? 14
		this.fontFamily = editorConfig.get("fontFamily") ?? "monospace"
	...
	async setKeyReservation(reserve) {
	...
	async performKeyReservation(reserve) {
		try {
	 ...
	 */
	async setupNextEditWindowManager(context, textApplier) {
	 ...
	 */
	updateCurrentCompletionId(completionId) {
	 ...
	 */
	registerCommandSafely(commandId, callback) {
		if (!this.context) {
	...
	async hideAllNextEditWindowsAndResetCompletionId() {
		await this.hideAllNextEditWindows()
	 ...
	 */
	async createCodeRender(text, currLineOffsetFromTop, newDiffLines, diffChars) {
		try {
	 ...
	 */
	async createCodeRenderDecoration(predictedCode, position, editableRegionStartLine, newDiffLines, diffChars) {
	 ...
	 */
	getDecorationOffsetPosition(editor, position) {
	 ...
	 */
	async renderWindow(editor, position, predictedCode, editableRegionStartLine, newDiffLines, diffChars) {
	...
	renderDeletions(editor, oldDiffChars) {
		const charsToDelete = []
		// const diffChars = myersCharDiff(oldEditRangeSlice, newEditRangeSlice);
...
//# sourceMappingURL=NextEditWindowManager.js.map
