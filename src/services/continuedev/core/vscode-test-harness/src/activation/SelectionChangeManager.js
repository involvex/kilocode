...
export class SelectionChangeManager {
	...
	initialize(ide, usingFullFileDiff) {
		this.ide = ide
	 ...
	 */
	registerListener(id, handler, priority = HandlerPriority.NORMAL) {
	 ...
	 * @param e The selection change event.
	 */
	async processEventWithTimeout(e) {
	...
	captureState(e) {
		return {
			nextEditWindowAccepted:
				NextEditWindowManager.isInstantiated() && NextEditWindowManager.getInstance().hasAccepted(),
	...
	async defaultFallbackHandler(e, _state) {
		if (!this.ide) {
...
//# sourceMappingURL=SelectionChangeManager.js.map
