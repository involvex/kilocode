...
const SVG_CONFIG = {
	...
	getFontSize() {
		return Math.ceil((vscode.workspace.getConfiguration("editor").get("fontSize") ?? 14) * 0.8)
	},
	getFontFamily() {
		return vscode.workspace.getConfiguration("editor").get("fontFamily") || "helvetica"
	},
	getPaddingX() {
...
export class JumpManager {
	...
	_jumpIcon
	_jumpDecoration
	_jumpDecorationVisible = false
	_disposables = []
	_jumpInProgress = false
	_jumpAccepted = false
	_completionAfterJump = null
	_oldCursorPosition
	constructor() {
  ...
  <text x="4" y="${textY}" font-family="${fontFamily}" font-size="${fontSize}" fill="${SVG_CONFIG.stroke}">${SVG_CONFIG.label}</text>
	...
	async suggestJump(currentPosition, nextJumpLocation, completionContent) {
		// Deduplication logic.
		...
		if (completionContent !== undefined) {
			...
			if (editor) {
				try {
					...
					const endLine = Math.min(startLine + completionLines.length - 1, document.lineCount - 1)
		...
		if (decorationLine < topLineNumber) {
			await this.renderTabToJumpDecoration(editor, topLineNumber, nextJumpLocation)
		} else if (decorationLine > bottomLineNumber) {
			await this.renderTabToJumpDecoration(editor, bottomLineNumber, nextJumpLocation)
		} else {
			// No suggestion is made when the decoration is within visibleRange.
			await this.renderTabToJumpDecoration(editor, decorationLine, nextJumpLocation)
		}
		return true
	}
	async renderTabToJumpDecoration(editor, lineToRenderOn, jumpPosition) {
	...
	async clearJumpDecoration() {
		...
		this.dispose()
		// Reset the context.
		await vscode.commands.executeCommand("setContext", "continue.jumpDecorationVisible", false)
		this._jumpDecorationVisible = false
	}
	async registerKeyListeners(editor, jumpPosition) {
		const acceptJumpCommand = vscode.commands.registerCommand("continue.acceptJump", async () => {
		...
		const rejectJumpCommand = vscode.commands.registerCommand("continue.rejectJump", async () => {
			if (this._jumpDecorationVisible) {
		...
		const selectionChangeListener = vscode.window.onDidChangeTextEditorSelection((e) => {
			...
			if (this._oldCursorPosition && !currentPosition.isEqual(this._oldCursorPosition)) {
...
export function __setMockJumpManagerInstance(mockInstance) {
