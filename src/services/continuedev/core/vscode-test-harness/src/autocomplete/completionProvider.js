...
export class ContinueCompletionProvider {
	...
	constructor(configHandler, ide, usingFullFileDiff) {
	 ...
	 */
	updateUsingFullFileDiff(usingFullFileDiff) {
		this.usingFullFileDiff = usingFullFileDiff
		this.prefetchQueue.initialize(this.usingFullFileDiff)
	}
	/**
	...
	async provideInlineCompletionItems(document, position, context, token) {
		...
		try {
			...
			let manuallyPassFileContents = undefined
			...
			const manuallyPassPrefix = undefined
			...
			if (isJumping && chainExists) {
				...
				if (this.isNextEditActive) {
					...
					logger.debug("Next edit outcome received", {
						...
						diffLinesLength: outcome?.diffLines?.length ?? 0,
					})
					if (
			...
			if (
				!outcome ||
				(!this.isNextEditActive && !outcome.completion) ||
				(this.isNextEditActive && !outcome.completion && outcome.diffLines.length === 0)
			) {
			...
			if (this.isNextEditActive) {
				this.nextEditProvider.markDisplayed(completionId, outcome)
			} else {
			...
			const editableRegionStartLine = outcome.editableRegionStartLine
			const editableRegionEndLine = outcome.editableRegionEndLine
			...
			const diffLines = outcome.diffLines
	...
	willDisplay(_document, selectedCompletionInfo, abortSignal, outcome) {
		if (selectedCompletionInfo) {
			const { text, range } = selectedCompletionInfo
