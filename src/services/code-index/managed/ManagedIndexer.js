...
export class ManagedIndexer {
	contextProxy
	static prevInstance = null
	static getInstance() {
	...
	fileUpsertLimit = pLimit(MANAGED_MAX_CONCURRENT_FILES)
	 ...
	 */
	getManagedIndexerStateSnapshot() {
		return {
	 ...
	 */
	async getManifest(state, branch, force = false) {
	 ...
	 */
	async processFiles(state, event, signal) {
		// Set indexing state
	...
	async search(query, directoryPrefix) {
		const { kilocodeOrganizationId, kilocodeToken } = this.config ?? {}
		if (!kilocodeOrganizationId || !kilocodeToken) {
	 ...
	 */
	async startScanForWorkspaceFolder(workspaceFolderPath) {
...
//# sourceMappingURL=ManagedIndexer.js.map
