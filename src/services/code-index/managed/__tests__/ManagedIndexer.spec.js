...
describe("ManagedIndexer", () => {
	...
	beforeEach(() => {
		...
		indexer.mockWatcherInstances = mockWatcherInstances
	})
	afterEach(() => {
		indexer.dispose()
	})
	...
	describe("dispose", () => {
		...
		it("should dispose workspaceFoldersListener if present", () => {
			...
			indexer.workspaceFoldersListener = { dispose: mockDispose }
			indexer.dispose()
		...
		it("should dispose configChangeListener", () => {
			const mockDispose = vi.fn()
			indexer.configChangeListener = { dispose: mockDispose }
			indexer.dispose()
			expect(mockDispose).toHaveBeenCalled()
	...
	describe("onEvent", () => {
		...
		describe("branch-changed event", () => {
			it("should fetch new manifest and process files", async () => {
				...
				vi.mocked(fs.promises.stat).mockResolvedValue({ size: 1000 })
				vi.mocked(apiClient.upsertFile).mockResolvedValue(undefined)
				const newManifest = {
					files: {},
				}
				vi.mocked(apiClient.getServerManifest).mockResolvedValue(newManifest)
				const mockFiles = async function* () {
			...
			it("should skip already indexed files", async () => {
				...
				vi.mocked(apiClient.getServerManifest).mockResolvedValue(manifestWithFile)
				const mockFiles = async function* () {
					yield { type: "file", filePath: "test.ts", fileHash: "abc123" }
				}
				const event = {
	...
	describe("abort mechanism", () => {
		it("should abort previous operation when new event arrives", async () => {
			...
			const mockFiles1 = async function* () {
			...
			const mockFiles2 = async function* () {
...
//# sourceMappingURL=ManagedIndexer.spec.js.map
