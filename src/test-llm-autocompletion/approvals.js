...
function getNextFileNumber(categoryDir, testName, type) {
...
function findMatchingFile(categoryDir, testName, type, content) {
	if (!fs.existsSync(categoryDir)) {
		return null
	}
	const pattern = new RegExp(`^${testName}\\.${type}\\.\\d+\\.txt$`)
...
async function askUserApproval(category, testName, input, output) {
...
export async function checkApproval(category, testName, input, output, skipApproval = false) {
