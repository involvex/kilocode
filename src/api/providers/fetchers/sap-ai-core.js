...
const modelProviderCache = {}
const PROVIDERS = ["Amazon", "Anthropic", "Google", "OpenAI", "Mistral AI"]
class SapAiCoreFetcher {
	...
	constructor(config) {
		...
		try {
			this.serviceKeyConfig = JSON.parse(this.config.serviceKey)
		} catch (error) {
	...
	filterRawModels(allModels) {
		return allModels.filter((model) => {
			...
			const latestVersion = model.versions?.find((version) => version.isLatest === true)
	...
	transformModels(allModels, deployments) {
		const result = {}
	...
	transformModel(model, deployments) {
		const latestVersion = model.versions?.find((version) => version.isLatest === true)
		if (!latestVersion) {
			return null
		}
	...
	createModelInfo(latestVersion, model) {
		const inputCost = latestVersion.cost?.find((c) => c.inputCost)?.inputCost
		const outputCost = latestVersion.cost?.find((c) => c.outputCost)?.outputCost
		return {
	...
	findModelDeployments(modelName, deployments) {
...
export async function getSapAiCoreModels(sapAiCoreServiceKey, sapAiCoreResourceGroup, sapAiCoreUseOrchestration) {
...
export async function getSapAiCoreDeployments(sapAiCoreServiceKey, sapAiCoreResourceGroup = "default") {
	const client = new SapAiCoreFetcher({
		serviceKey: sapAiCoreServiceKey,
		resourceGroup: sapAiCoreResourceGroup,
	})
...
function filterModelsByScenario(allModels, sapAiCoreUseOrchestration) {
