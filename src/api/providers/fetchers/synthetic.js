...
function parseSyntheticModel(model) {
...
export async function getSyntheticModels(apiKey) {
	const models = {}
	try {
		const headers = {
		...
		const response = await axios.get("https://api.synthetic.new/openai/v1/models", {
			headers,
			timeout: 10_000,
		})
