import { getModelId, nativeFunctionCallingProviders, TOOL_PROTOCOL } from "@roo-code/types"
...
export function getActiveToolUseStyle(settings) {
	const workspaceSetting =
		"workspace" in vscode
			? vscode.workspace.getConfiguration(Package.name).get("toolProtocol", TOOL_PROTOCOL.XML)
			: TOOL_PROTOCOL.XML
	if (!settings) {
		console.error("getActiveToolUseStyle: settings missing, returning", workspaceSetting)
		return workspaceSetting
	}
	if (settings.apiProvider && !nativeFunctionCallingProviders.includes(settings.apiProvider)) {
	...
	if (
		providersDefaultingToNativeKeywords.includes(settings.apiProvider) ||
 ...
 */
export function addNativeToolCallsToParams(params, options, metadata) {
...
export function convertOpenAIToolsToAnthropic(allowedTools) {
...
//# sourceMappingURL=nativeToolCallHelpers.js.map
