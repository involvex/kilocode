...
describe("GitWatcher", () => {
	let config
	let mockExecGetLines
	let mockGetCurrentBranch
	let mockGetCurrentCommitSha
	let mockGetGitHeadPath
	let mockIsDetachedHead
	let mockGetBaseBranch
	let mockGetGitDiff
	beforeEach(() => {
		config = {
			cwd: "/test/repo",
	...
	describe("onEvent", () => {
		it("should register an event handler", async () => {
			...
			watcher.emitter.emit("event", testEvent)
		...
		it("should emit branch-changed events with files iterable", async () => {
			...
			watcher.emitter.emit("event", branchChangedEvent)
		...
		it("should emit commit events with files iterable", async () => {
			...
			watcher.emitter.emit("event", commitEvent)
	...
	describe("file iteration", () => {
		it("should yield all files on base branch", async () => {
			...
			const files = []
			for await (const file of watcher.getAllFiles()) {
				files.push(file)
			}
		...
		it("should yield diff files on feature branch", async () => {
			...
			const files = []
			for await (const file of watcher.getDiffFiles("feature/test", "main")) {
				files.push(file)
			}
...
//# sourceMappingURL=GitWatcher.spec.js.map
