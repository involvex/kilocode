...
function resetSessionManagerInstance() {
	SessionManager.instance = null
}
describe("SessionManager", () => {
	let mockPathProvider
	let mockLogger
	let mockExtensionMessenger
	...
	beforeEach(() => {
		...
		mockPathProvider = {
			...
			getSessionFilePath: vi.fn().mockImplementation((workspaceDir) => `${workspaceDir}/.kilocode/session.json`),
		}
		mockLogger = {
			debug: vi.fn(),
			info: vi.fn(),
		...
		vi.mocked(SessionPersistenceManager).mockImplementation(() => mockSessionPersistenceManager)
	})
	afterEach(() => {
		vi.useRealTimers()
		resetSessionManagerInstance()
	...
	describe("getFirstMessageText", () => {
		...
		it("should normalize whitespace", () => {
			const manager = SessionManager.init(mockDependencies)
			const messages = [{ ts: 123, type: "say", say: "text", text: "  Hello   \n  World  " }]
			const result = manager.getFirstMessageText(messages)
			expect(result).toBe("Hello World")
		})
		it("should truncate long text when truncate is true", () => {
			...
			const messages = [{ ts: 123, type: "say", say: "text", text: longText }]
		...
		it("should not truncate text at 140 characters or less", () => {
			const manager = SessionManager.init(mockDependencies)
			const messages = [{ ts: 123, type: "say", say: "text", text: "A".repeat(140) }]
			const result = manager.getFirstMessageText(messages, true)
			expect(result).toHaveLength(140)
	...
	describe("generateTitle", () => {
		...
		it("should return raw text when it is 140 characters or less", async () => {
			const manager = SessionManager.init(mockDependencies)
			const messages = [{ ts: 123, type: "say", say: "text", text: "Short title" }]
	...
	describe("syncSession", () => {
		async function triggerSyncAndWait(manager) {
			if (manager["timer"]) {
