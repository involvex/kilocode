 ...
 */
export async function getKeybindingsForCommands(commandIds) {
	const keybindings = {}
...
export async function getKeybindingForCommand(commandId) {
	try {
		const userKeybindings = await readUserConfigFile("keybindings.json")
		const userBinding = findAnyBindingForCommand(userKeybindings, commandId)
 ...
 */
export function getDefaultKeybindingForCommand(commandId) {
...
function findAnyBindingForCommand(entries, commandId) {
	return entries.find((entry) => entry.command === commandId)
}
function prettyPrintKey(rawKey, platform) {
 ...
 */
function mapModifierKey(key, platform) {
 ...
 */
function normalizeKeyToken(token, platform) {
