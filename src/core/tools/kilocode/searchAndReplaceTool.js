...
import { SearchAndReplaceParametersSchema } from "../../prompts/tools/native-tools/kilocode/search_and_replace"
/**
 ...
 */
async function validateParams(cline, block, pushToolResult) {
 ...
 */
export async function searchAndReplaceTool(cline, block, askApproval, handleError, pushToolResult) {
	try {
		if (block.partial) {
			return
		}
		...
		if (validRelPath) {
			await cline.fileContextTracker.trackFileContext(validRelPath, "roo_edited")
		}
