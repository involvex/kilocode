...
export const webviewMessageHandler = async (
	provider,
	message, // kilocode_change switch to MaybeTypedWebviewMessage for better type-safety
	marketplaceManager,
) => {
	...
	switch (message.type) {
						...
						if (isNewMode) {
							...
							const changedSettings = existingMode
								? Object.keys(message.modeConfig).filter(
										(key) =>
											JSON.stringify(existingMode[key]) !==
		...
		case "updateGlobalState": {
			const { stateKey, stateValue } = message
