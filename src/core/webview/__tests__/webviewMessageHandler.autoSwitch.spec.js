...
describe("webviewMessageHandler - Automatic Organization Switching", () => {
	let mockProvider
	let mockGetGlobalState
	let mockUpdateGlobalState
	let mockUpsertProviderProfile
	let mockPostMessageToWebview
	let mockPostStateToWebview
	let mockLog
	...
	describe("Auto-Switch Success Cases", () => {
		it("should auto-switch to first organization on first login", async () => {
			...
			mockGetGlobalState.mockImplementation((key) => {
			...
			getModels.mockResolvedValueOnce({ "model-1": {} })
		...
		it("should fetch organization modes after auto-switch", async () => {
			...
			const callArgs = refreshOrganizationModes.mock.calls[0]
		...
		it("should flush and refetch models after auto-switch", async () => {
			...
			mockGetGlobalState.mockReturnValue(undefined)
			axios.get.mockResolvedValueOnce({ data: mockProfileData })
			getModels.mockResolvedValueOnce({ "model-1": {}, "model-2": {} })
			await webviewMessageHandler(mockProvider, {
				type: "fetchProfileDataRequest",
			})
	...
	describe("Error Handling Cases", () => {
		...
		it("should handle organization modes fetch failure gracefully", async () => {
			...
			axios.get.mockResolvedValueOnce({ data: mockProfileData })
			getModels.mockResolvedValueOnce({ "model-1": {} })
			refreshOrganizationModes.mockRejectedValueOnce(new Error("Network error"))
			await webviewMessageHandler(mockProvider, {
				type: "fetchProfileDataRequest",
			})
	...
	describe("Integration Cases", () => {
		...
		it("should complete full auto-switch flow in correct order", async () => {
			...
			refreshOrganizationModes.mockImplementation(async () => {
				callOrder.push("refreshOrganizationModes")
			})
			flushModels.mockImplementation(async () => {
				callOrder.push("flushModels")
			})
			getModels.mockImplementation(async () => {
				callOrder.push("getModels")
				return { "model-1": {} }
			})
			mockUpdateGlobalState.mockImplementation(async (key) => {
...
//# sourceMappingURL=webviewMessageHandler.autoSwitch.spec.js.map
