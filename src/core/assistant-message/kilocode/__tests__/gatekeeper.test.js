...
describe("gatekeeper", () => {
	...
	describe("evaluateGatekeeperApproval", () => {
		it("should return true when no gatekeeper is configured", async () => {
			...
			const result = await evaluateGatekeeperApproval(mockTask, "read_file", { path: "test.ts" })
		...
		it("should approve when gatekeeper responds with 'yes'", async () => {
			...
			const result = await evaluateGatekeeperApproval(mockTask, "write_to_file", {
				path: "test.ts",
				content: "test",
			})
			expect(result).toBe(true)
		...
		it("should deny when gatekeeper responds with 'no'", async () => {
			...
			const result = await evaluateGatekeeperApproval(mockTask, "execute_command", {
				command: "rm -rf /",
			})
		...
		it("should display cost when usage information is available", async () => {
			...
			await evaluateGatekeeperApproval(mockTask, "write_to_file", {
				path: "test.ts",
				content: "test",
			})
			expect(mockTask.say).toHaveBeenCalledWith(
		...
		describe("buildGatekeeperPrompt", () => {
			...
			it("should build prompt for execute_command tool", async () => {
				...
				await evaluateGatekeeperApproval(mockTask, "execute_command", {
					command: "npm test",
					cwd: "/test/dir",
				})
				expect(streamResponseFromHandler).toHaveBeenCalledWith(
			...
			it("should pre-approve read_file tool", async () => {
				const result = await evaluateGatekeeperApproval(mockTask, "read_file", {
					path: "test.ts",
				})
				expect(result).toBe(true)
			...
			it("should build prompt for browser_action tool", async () => {
				...
				await evaluateGatekeeperApproval(mockTask, "browser_action", {
					action: "launch",
			...
			it("should build prompt for use_mcp_tool", async () => {
				...
				await evaluateGatekeeperApproval(mockTask, "use_mcp_tool", {
					server_name: "github",
			...
			it("should pre-approve update_todo_list", async () => {
				const result = await evaluateGatekeeperApproval(mockTask, "update_todo_list", {})
				expect(result).toBe(true)
