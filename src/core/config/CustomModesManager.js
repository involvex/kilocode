...
export class CustomModesManager {
	...
	async mergeCustomModes(projectModes, globalModes, organizationModes = []) {
		const slugs = new Set()
		const merged = []
	...
	async watchCustomModesFiles() {
		...
		const handleSettingsChange = async () => {
			try {
				...
				const storedModes = (await this.context.globalState.get("customModes")) || []
	 ...
	 */
	async fetchOrganizationModes(kilocodeToken, organizationId, kilocodeTesterWarningsDisabledUntil) {
		try {
	 ...
	 */
	async refreshWithOrganizationModes(organizationModes) {
