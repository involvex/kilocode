...
export class Task extends EventEmitter {
	...
	async sayAndCreateMissingParamError(toolName, paramName, relPath) {
		...
		await this.say(
			"error",
			`Kilo Code tried to use ${toolName}${relPath ? ` for '${relPath.toPosix()}'` : ""} without value for required parameter '${paramName}'. ${kilocodeExtraText}Retrying...`,
		)
		return formatResponse.toolError(
			formatResponse.missingToolParameterError(paramName, getActiveToolUseStyle(this.apiConfiguration)),
		)
	}
	// Lifecycle
	...
	async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
		...
		while (stack.length > 0) {
			...
			try {
				...
				// kilocode_change start
				const reasoningDetails = new Array()
				...
				if (isNative) {
					...
					const nativeToolBlocks = this.assistantMessageContent.filter(
						(block) => block.type === "tool_use" && block.id !== undefined,
					)
