...
import { TaskCommandName, RooCodeEventName, IpcMessageType, EVALS_SETTINGS } from "@roo-code/types"
import { IpcClient } from "@roo-code/ipc"
import { findRun, findTask, updateTask, createTaskMetrics, updateTaskMetrics, createToolError } from "../db/index.js"
...
export const processTask = async ({ taskId, logger }) => {
...
export const processTaskInContainer = async ({ taskId, logger, maxRetries = 10 }) => {
	const baseArgs = [
		"--rm",
...
export const runTask = async ({ run, task, publish, logger }) => {
	const { language, exercise } = task
