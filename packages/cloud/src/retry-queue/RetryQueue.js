...
export class RetryQueue extends EventEmitter {
	...
	isProcessing = false
	retryTimer
	STORAGE_KEY = "roo.retryQueue"
	authHeaderProvider
	queuePausedUntil // Timestamp when the queue can resume processing
	isPaused = false // Manual pause state (e.g., for auth state changes)
	currentUserId // Track current user ID for conditional clearing
	hasHadUser = false // Track if we've ever had a user (to distinguish from first login)
	constructor(context, config, log, authHeaderProvider) {
		super()
	...
	async enqueue(url, options, type = "other", operation) {
			...
			for (const request of requests) {
				try {
					...
					if (this.config.maxRetries > 0 && request.retryCount >= this.config.maxRetries) {
						...
						this.queue.delete(request.id)
						this.emit("request-max-retries-exceeded", request, error)
					} else {
						this.queue.set(request.id, request)
						this.emit("request-retry-failed", request, error)
					}
